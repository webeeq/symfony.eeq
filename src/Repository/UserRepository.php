...
class UserRepository extends ServiceEntityRepository
{
    ...
    public function setUserPassword(
        ...
        $query = $this->getEntityManager()->createQuery(
            ...
            ->setParameter('key', md5(date('Y-m-d H:i:s') . $password))
            ->setParameter('ip', $ip)
    ...
    public function isIdPassword(int $id, string $password): ?object
    {
        $query = $this->getEntityManager()->createQuery(
            'SELECT u FROM App:User u
    ...
    public function isUserPassword(string $login, string $password): ?object
    {
        $query = $this->getEntityManager()->createQuery(
            'SELECT u FROM App:User u
    ...
    public function isRestUserPassword(
        string $login,
        string $password
    ): ?object {
        $query = $this->getEntityManager()->createQuery(
            'SELECT u FROM App:User u
            WHERE u.active = 1 AND u.login = :login AND u.password = :password'
        )
            ->setParameter('login', $login)
